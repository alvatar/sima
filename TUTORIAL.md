...
This tutorial takes a program of which sima knows nothing and conducts it through a complete search: we shall install the instrument, execute the work, inspect what it produced, alter the code, and come to understand precisely what such an alteration does to the results already resting in the store.
...
**Scope.** Everything here proceeds upon one machine until the later sections, in which a whole search removes to another, and a company of machines is engaged to serve one. See [Running on other machines](#running-on-other-machines).
...
- **Search** — one search, declared by a `sima.toml`. Its id is the hash of the config's identity-bearing fields.
...
- **Store** — content-addressed objects together with a journal kept for each search. The store holds the results; the journal narrates how they were produced.
...
- `translate_config(toml, segmented)` — renders the `[search.params]` table into canonical bytes.
...
- The **domain** is interrogated where the search is conducted: what devices exist, what environment identifies the results, how the config is to be translated. This must be had cheaply — no device, no loaded assets.
...
`Domain.executor(device)` is therefore a **constructor**. The domain answers questions anywhere; the executor performs work upon a device. Task keys include the environment, and this is why their derivation must demand no device: a laptop possessing no GPU may nonetheless derive every key in a search.
...
- `[search.generator] id = "..."` is matched against the `id()` of each generator served. Where no match is found, the config is in error, and the id is named in the complaint.
- The matched generator's `format()` must equal the search's format, or it is refused with both ids named.

To pass several generators to `serve` is the means by which one format acquires several strategies of search — a sweep, a random sample, a mutation of a former search's winners. The generator's id and its params bear identity, so two strategies keep their task keys apart.
...
- **`[search]`** is the identity: seed, format, and `segments = 3`, which divides each candidate's work into a chain of 3 tasks, each continuing from the committed state of the last.
- **`[search.generator]`** names the generator; the keys following `id` belong to your program and cross as TOML text.
- **`[search.params]`** belongs to your program likewise.
...
```
search bff61aa384aa94add7c1609ae6634ca0825ac9548b69712563af224e18b800ef
started: 6 tasks
...
An attempt says when it begins and which worker took it; a task that saves checkpoints says so as it goes, at most once in ten seconds however often it saves. Between them the terminal shows a search computing rather than a search that has merely not finished. `--quiet` leaves the search, its start, its commits, and its ending.

Six tasks is `count = 2` candidates × `segments = 3`. The config decided that; the program did not. As shipped, the search computes for some couple of minutes; reduce `steps` by a few orders of magnitude if you wish merely to see the shape of the output.
...
**`sima status`** reports the state of the search:
...
Two distinct values, three tasks apiece — one group to each candidate, one task to each segment. `sima report search.toml --all` prints a line for every task, and `--timeline` reports the search's metrics as they unfolded in time.
...
- `sima tui search.toml` — the search conducted in a full-screen terminal UI.
...
The search is **refused**: sima recorded the binary's digest when the search began, and the file no longer answers to it. This is a guard upon provenance — it exists so that code cannot be exchanged silently beneath a half-finished search. Override it deliberately:

```
sima search search.toml --accept-binary
```

Nothing recomputes. The search is finalized, every task key is already answered, and `sima report search.toml --all` shows the old values. **New code, old results.**
...
- `sima rm <config>` — delete the search and whatever only it references.
- `sima pack <store-dir>` — consolidate loose objects into packs.
- `sima pack <store-dir> --gc` — delete, besides, everything outside the closures of the finalized searches; this destroys the work of any search still in progress.

## Running on other machines

sima has two workflows for placing a search upon other hardware.

- **Migrate** (`sima migrate <config>`) — the search's durable state travels to one declared host, a `sima search` process conducts it there, and your machine holds the lock and follows. **Your program travels with it**, which is the matter of the remainder of this section.
- **Fleet** (`sima search <config> --fleet`) — the store and the orchestrator remain upon your machine; declared or rented machines run workers only. Your program travels there as well: the same `payload` key sends it to every machine the fleet draws in, each installs it beneath its `root`, and its workers run what was installed. Both are driven below.

### Declaring what travels

A migration moves the search onto a machine that has never seen your program, and the program must therefore travel with it. Say what, upon the entry:
...
An entry stating no `payload` describes a program which this machine holds and no other; both `sima migrate` and `sima search --fleet` refuse it, naming the missing key.
...
A script that exits with any figure but zero, or leaves no entry point, fails the search upon the destination — and `sima migrate` reports the machine and the last lines of your script's own complaint, so that you read the failure here.

### The journey: one search, three machines

Everything to this point has run upon your machine. A real search outgrows one — and this section therefore takes the search upon the journey a real one makes: it begins upon your laptop, removes to a rented machine, computes there while you are absent, and comes home to finish. One search id the whole way, and no committed task is ever computed twice.

The example is already provisioned for the trip: `steps` gives each task minutes of computation rather than instants, and `checkpoint_interval_ms = 2000` means that every couple of seconds each task lays by state from which it may resume — which is what permits this search to change machines in the middle of a task.

**Start local.** `sima search search.toml`; suffer a couple of tasks to commit, then Ctrl-C. The search winds down — the attempts in flight are abandoned, their checkpoints standing where they were laid — and exits `130`: interrupted, resumable. Your store now holds a search that is partly done, and running it again resumes each abandoned attempt from its checkpoint rather than from its beginning.

**Send it away.** The example's config already declares whither:

```toml
[orchestrator]
migrate = "cloudbox"   # the [host.*] entry the search moves onto
...
[budget]
max_spend_usd     = 2.0     # wind the search down if rental spend reaches this
...
sima rents the cheapest machine that satisfies the constraints, boots its image upon it, and ships the search — the store's objects, your program, the whole of it. Each phase names itself as it begins, so that the quiet minutes while the machine comes up are quiet for a stated reason:
...
This is the destructive verb, and the only one. It winds the far search down, reads how it ended, draws everything it computed into your store, and takes the rented machine away:

```
migrated: search 4728422a… came home with 2 tasks outstanding
```

A rental bills by the hour and not by what it computes, so to stop the search early saves you nothing — the same bill arrives whether the machine computes or sits idle — and to tear the machine down requires the provider credential, which never leaves your machine. That is why recall does both at one stroke, and why nothing remote can do it in your stead. `sima reconcile search.toml` is the net beneath it all: it asks the provider what still stands rented under your key, and answers `nothing to reconcile` when the ledger is clean.

**Finish at home.** `sima search search.toml` one last time. The store knows what remains outstanding; the remaining tasks compute upon your laptop, and the search reports `finalized`. It began here, computed upon a machine that no longer exists, and ended here — and the manifest is the very one any single machine would have written.

**Settle the account.** `sima report search.toml --spend` states what the rental cost, and `sima report search.toml --machines` the reputation of the machines that served — a machine that misbehaved is blacklisted and not rented again. While a search lives elsewhere, `sima status`, `sima report`, `sima tui`, and `sima follow` all accept `--on <host>` to observe it upon its own machine; and where a migrated search's machine is to be destroyed without the ceremony of a recall — nothing upon it worth bringing home, the rental merely billing — `sima reconcile search.toml --hosted` takes that one down as well; plain `reconcile` spares it.

### Starting over

`root_seed` in `[search]` is the anchor of the identity, and your generator receives it as the first argument of `generate` — one seed always yields the same candidates. To rerun a finalized search is to find its commits and do nothing; to run the same search afresh, advance the seed — a new identity, every task under new addresses. To recompute under the *same* seed, clean up first (below).
...
`[fleet]` lists what a search *may* draw upon; nothing is rented until you ask:
...
Each member says what it took and at what rate the moment its offer is taken, which is minutes before that machine is up; the wait line states what those minutes are spent upon, and the online line marks the boot completed. Should a member fail to come up, it says so and states what your `fill` policy makes of it — the search stops under `strict`, and goes on with the machines that did come up under `best-effort`. Every member that fell short says so, since they were all asked for together. Pass `--quiet` and none of it is printed: what remains is the search, its start, its commits, and its ending.

Change your mind while the machines are still coming up, and one Ctrl-C suffices. The acquisition stops where it stands, every machine already rented is released, and the search exits `130` — nothing executed, your store as it was, and a line saying how many machines went back. You need neither wait out a boot nor kill the process over one.

Your program went to both machines just as it traveled in the migration; each installed it, and its workers ran what was installed. When the search ends, the rentals are torn down of their own accord — `sima reconcile search.toml` confirms the ledger clean.

What has just happened deserves understanding in three particulars:

- **Parallelism is purchased with `count` in the generator, not with machines.** The segments of one candidate are sequential by construction — each begins from the committed state of the last — so at most one task per candidate is ever ready. With two candidates, two machines is already more than the search can feed; twenty candidates would keep every worker on all of them occupied.
...
`sima rm` deletes one search — the search the config presently names — together with everything only it references:

```
sima rm search.toml
```

Mark that "presently names": the target is resolved through the config. A store accumulates the searches of every identity ever driven against it — an edited seed, a changed parameter — and what it holds is asked of the store rather than of a config:
...
Any search in the listing is removed by its own id, whether or not the config still names it — any unambiguous prefix will serve:

```
sima rm search.toml --search 4728422a
```

Ask for a search the store never held, and it refuses by name:

```
sima: validation error: cannot remove search 7c19fe97…: search not found
```

`rm -rf .sima/` removes every search at once, together with the rest of what the config generated, all of it being disposable in its entirety. After the cleaning, by either instrument, the same seed computes afresh from nothing — whereas without it, running again finds its commits and does nothing.

### A deadline where time is free

A local search and a machine of your own cost nothing by the hour, so there a deadline is worth stating:
...
The search interrupts itself after six hours, whether or not anything watches, and leaves the store resumable. The key does not travel to a rented destination.

### Changing the program

Edit your payload and run `sima migrate` again: the new manifest travels and the destination installs it. The far search then stops, for its stored results and checkpoints came of the previous build, and sima cannot judge whether the change was material. That judgment is yours to make:

```
sima migrate search.toml --accept-binary
```

The flag travels to the search upon the destination, which is where the comparison is made.
