//! Resume-checkpoint slots: mutable per-chain scratch state under a run.
//!
...
impl Store {
    ...
    pub fn save_checkpoint(
        ...
        atomic::create_dir_durable(&layout::checkpoint_dir(self.root(), run))?;
        let mut enc = Enc::new();
        enc.str(TAG_CHECKPOINT).hash(key.as_hash()).bytes(payload);
        let path = layout::checkpoint_path(self.root(), run, slot);
    ...
    /// failure is `Err`.
    pub fn checkpoint(&self, run: &RunId, slot: u64, key: &TaskKey) -> Result<Option<Vec<u8>>> {
        let path = layout::checkpoint_path(self.root(), run, slot);
...
#[cfg(test)]
mod tests {
    use crate::testutil::{sample_identity, sample_run_config, temp_store};
    ...
    fn save_then_load_round_trips_at_the_pinned_path() -> Result<()> {
        ...
        store.save_checkpoint(&run, 3, &key, b"continuation bytes")?;
        assert_eq!(
            store.checkpoint(&run, 3, &key)?.as_deref(),
    ...
    fn a_missing_slot_loads_as_none() -> Result<()> {
        ...
        let run = sample_run_config(42).id();
        let key = sample_identity(1).key();
        assert_eq!(store.checkpoint(&run, 0, &key)?, None);
        Ok(())
    }
    ...
    fn a_second_save_overwrites_the_first() -> Result<()> {
        ...
        let run = sample_run_config(42).id();
        let key = sample_identity(1).key();
        store.save_checkpoint(&run, 0, &key, b"first")?;
        store.save_checkpoint(&run, 0, &key, b"second")?;
        assert_eq!(
            store.checkpoint(&run, 0, &key)?.as_deref(),
    ...
    fn a_slot_keyed_to_another_task_loads_as_none() -> Result<()> {
        ...
        store.save_checkpoint(&run, 0, &previous, b"old state")?;
        assert_eq!(store.checkpoint(&run, 0, &current)?, None);
        // The owning key still reads it.
        assert_eq!(
            store.checkpoint(&run, 0, &previous)?.as_deref(),
    ...
    fn a_corrupted_slot_loads_as_none() -> Result<()> {
        ...
        let run = sample_run_config(42).id();
        let key = sample_identity(1).key();
        store.save_checkpoint(&run, 0, &key, b"payload")?;
        let path = dir
            .path()
            .join("runs")
            .join(run.to_string())
    ...
    fn slots_are_independent_per_chain() -> Result<()> {
        ...
        store.save_checkpoint(&run, 0, &key_a, b"chain zero")?;
        store.save_checkpoint(&run, 1, &key_b, b"chain one")?;
        assert_eq!(
            store.checkpoint(&run, 0, &key_a)?.as_deref(),
            Some(b"chain zero".as_slice())
        );
        assert_eq!(
            store.checkpoint(&run, 1, &key_b)?.as_deref(),
