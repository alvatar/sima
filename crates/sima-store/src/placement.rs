//! Placement slots: mutable per-chain device bindings under a run.
//!
...
//! [`Store::chain_bindings`] reads a run's slots back for resume — and there is
...
impl Store {
    ...
    pub fn bind_chain(&self, run: &RunId, chain: u64, payload: &[u8]) -> Result<()> {
        atomic::create_dir_durable(&layout::placement_dir(self.root(), run))?;
        let mut enc = Enc::new();
        enc.str(TAG_PLACEMENT).bytes(payload);
        let path = layout::placement_path(self.root(), run, chain);
        atomic::write_atomic(self.root(), &path, &enc.finish())
    }

    /// Every chain binding the run holds, for seeding placement on resume. A
    /// run with no slots — and a run whose placement directory was never
    ...
    pub fn chain_bindings(&self, run: &RunId) -> Result<HashMap<u64, Vec<u8>>> {
        let dir = layout::placement_dir(self.root(), run);
...
mod tests {
    ...
    fn a_binding_round_trips_at_the_pinned_path() -> Result<()> {
        ...
        store.bind_chain(&run, 3, br#"{"vendor_id":32902,"device_id":32081}"#)?;
        assert_eq!(
            store.chain_bindings(&run)?.get(&3).map(Vec::as_slice),
    ...
    fn a_run_with_no_bindings_reads_empty() -> Result<()> {
        ...
        let run = sample_run_config(42).id();
        assert!(store.chain_bindings(&run)?.is_empty());
        Ok(())
    }
    ...
    fn a_rebind_overwrites_the_previous_binding() -> Result<()> {
        let (_dir, store) = temp_store();
        let run = sample_run_config(42).id();
        store.bind_chain(&run, 0, b"first class")?;
        store.bind_chain(&run, 0, b"second class")?;
    ...
    fn every_chain_of_a_run_reads_back() -> Result<()> {
        ...
        let run = sample_run_config(42).id();
        for chain in 0..4u64 {
            store.bind_chain(&run, chain, format!("class {chain}").as_bytes())?;
        }
        let bindings = store.chain_bindings(&run)?;
    ...
    fn bindings_are_scoped_to_their_run() -> Result<()> {
        let (_dir, store) = temp_store();
        let one = sample_run_config(1).id();
        let two = sample_run_config(2).id();
    ...
    fn a_malformed_slot_is_skipped() -> Result<()> {
        ...
        // chain binds again rather than the run failing.
        let (dir, store) = temp_store();
        let run = sample_run_config(42).id();
        store.bind_chain(&run, 0, b"intact")?;
        let placement = dir
            .path()
            .join("runs")
            .join(run.to_string())
    ...
    fn a_file_that_does_not_name_a_chain_is_skipped() -> Result<()> {
        ...
        assert_eq!(store.chain_bindings(&run)?.len(), 1);
        Ok(())
    }
}
