//! The watchdog: it detects expired leases and reports them.
//!
...
//! lease table and emits one `LeaseExpired` event per lease whose age exceeds
//! `attempt_timeout`, never touching the lease or the worker.
...
pub(crate) fn watchdog_loop(
    ...
    // the same task can expire and report afresh.
