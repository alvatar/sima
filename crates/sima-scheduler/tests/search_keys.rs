//! `search_keys`: the task keys a search comprises, derived from `(config, store
...
//! search exactly what its manifest lists. A store sync needs that set on both
...
use common::{chained_config, config, environment, exec, run_into, search_controlled, temp_store};
use sima_core::Result;
use sima_domains::{StubBehavior, StubGenerator};
use sima_model::TaskKey;
use sima_scheduler::{Event, Record, SearchControl, SearchOutcome, search_keys};
...
fn a_partly_committed_chain_yields_the_keys_it_has_materialized() -> Result<()> {
    ...
    // beyond the frontier.
    //
    // How far the search got before the interrupt landed is not pinned: the
    ...
    let outcome = search_controlled(&store, &cfg, &exec(1, 1, 5_000), &control)?;
    ...
    assert!(
        prefix >= 2,
        "the search committed before it was interrupted"
    );
...
fn deriving_the_keys_twice_over_one_store_answers_the_same() -> Result<()> {
    // Constructing a source writes the search's spec objects, which is idempotent:
...
#[test]
fn over_a_finalized_search_the_keys_are_exactly_what_the_manifest_lists() -> Result<()> {
    // The regression the sync rests on: after a search finalizes, the derivation
    ...
    let manifest = store
        .manifest(&cfg.id())?
        .expect("a finalized search has a manifest");
