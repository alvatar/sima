//! The registry: a format or generator id resolved to one of this domain's
//! models.
//!
...
use super::domain::CaDomain;
use super::model::CaModel;
use super::models::gray_scott::GrayScott;
use super::models::gray_scott_cuda::GrayScottCuda;
use super::models::nca::Nca;
...
pub(crate) fn generator_for(id: &GeneratorId) -> Option<Result<Box<dyn Generator>>> {
    ...
    fn drawn<M: CaModel>() -> Result<Box<dyn Generator>> {
        Ok(Box::new(super::generator::CaGenerator::<M>::new()?))
    }
    match id.as_str() {
...
mod tests {
    use super::*;
    use crate::domains::ca_evolution::{domain_for, generator_for};
