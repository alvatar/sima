...
//! channel.
//!
//! [`grid_stats`] summarizes the final grid into canonical bytes. The bytes
//! travel only the observational path — executor `Stats` to the journal — and
//! never enter a record, a manifest, or any identity criterion; [`Enc`] is used
//! because the summary is structured, and a byte pin keeps the format stable.
