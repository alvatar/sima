//! [`build_domain`]: assembles a [`Domain`] from a CA model and the substrate
//! it runs on.
...
/// blake3 digest of the update-kernel source, the digest of the substrate's
/// reduction kernel, and the substrate's pinned compiler identity. The first
...
/// kernel's does. Two substrates give one rule two distinct environments, so
