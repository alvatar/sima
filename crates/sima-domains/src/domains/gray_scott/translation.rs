//! The Gray-Scott config translation: the `[run.generator]` section into
//! canonical [`GrayScottGeneratorConfig`] bytes, and the binding of the
//! domain's id.

use sima_core::{Error, Result};

use super::GrayScottGeneratorConfig;
...
/// The value at `key`; a missing key is [`Error::Validation`] naming it.
fn required<'t>(table: &'t toml::Table, key: &str) -> Result<&'t toml::Value> {
    table.get(key).ok_or_else(|| {
        Error::Validation(format!("generator gray-scott.v1 requires the key {key:?}"))
    })
}
