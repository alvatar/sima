...
/// `+0.0`), read at f64 as the config file's one number path.
pub(crate) fn float(table: &toml::Table, id: &str, section: &str, key: &str) -> Result<f64> {
    match required(table, id, section, key)? {
...
/// (a `0` means `+0.0`), converted as f64 then f32 — the config file's one
/// number path.
...
/// The required `[run.generator]` range at `key`: a two-element `[lo, hi]` array
/// of numbers. A fixed parameter is spelled as the degenerate range `[v, v]`.
pub(crate) fn range(table: &toml::Table, id: &str, key: &str) -> Result<[f32; 2]> {
    let value = required(table, id, "generator", key)?;
    ...
    Err(Error::Validation(format!(
        "generator {id} {key} must be a two-element [lo, hi] array of numbers, got {}",
        value.type_str()
    )))
}
