//! CLI acceptance for the retention policy's floor: what a finalized run
...
//! run's snapshot objects away and asking the binary for the run again.
...
/// Every artifact object the run's records reference, deduplicated — the
...
fn snapshots(config: &std::path::Path) -> BTreeSet<Hash> {
    ...
    let manifest = store
        .manifest(&loaded.run.id())
        .expect("read manifest")
        .expect("the run finalized");
    let mut objects = BTreeSet::new();
    for entry in &manifest.entries {
        let record = store
            .record(&entry.task)
            .expect("read record")
            .expect("a finalized run's tasks are committed");
...
#[test]
fn a_finalized_run_re_evaluates_after_its_snapshots_are_deleted() {
    ...
    let manifest = manifest_bytes(&config).expect("the run finalized");
    let commits = committed(&config);
    assert!(commits > 0, "the run committed its tasks");
    ...
    // store offers no single-object delete — deletion is run-grained and
    ...
    // for it. The run was never packed, so every object is loose at
    // `objects/<aa>/<hash>`.
    ...
    // The run is asked for again. The frontier re-derives from the task
    // index and the records, both untouched, so the run re-finalizes without
    // executing anything.
    let again = sima(&["run", path]);
    ...
    assert_eq!(
        manifest_bytes(&config).expect("the run stays finalized"),
        manifest,
        "the manifest is byte-identical"
    );

    // The query commands answer for the run too: neither reads a snapshot.
