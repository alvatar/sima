//! [`SearchStatus`]: a search's observable state, built from its lifecycle events.
...
/// A search's observable state, built from its lifecycle events. `sima status`
/// and the tui update the same `SearchStatus` type through the same
/// [`apply`](SearchStatus::apply) method, so both derive identical state from the
/// same events: `status` replays a stored journal, and the tui applies each
/// observer event as it arrives while a search proceeds.
#[derive(Debug, PartialEq, Eq)]
pub struct SearchStatus {
    /// The search the status describes.
    pub search: SearchId,
    /// The search's task count, from the latest `SearchStarted` — resume appends
    ...
    /// one device name on two machines counts separately. The search's
    ...
    /// for a journal carrying no `WorkerBound` events — a search whose domain uses
    ...
    /// failed event clears it, and a segment's `SearchStarted` empties the map.
...
/// The state the journal's last search-level event decides. A journal ending
/// mid-search reads as in progress: a dead orchestrator is indistinguishable
/// from a live one by the journal alone.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum SearchState {
    /// A segment started and no search-level event closed it.
    InProgress,
    /// The search finalized; its manifest is written.
    Finalized,
    /// A definitive candidate failure ended the search.
    Failed {
    ...
    /// The caller interrupted the search; the store is resumable.
    Interrupted,
}

impl SearchStatus {
    /// A zeroed status for `search`: no events applied, `InProgress`, no leases
    /// held. Applying a journal or a live event stream through
    /// [`apply`](SearchStatus::apply) drives it to the search's observable state.
    pub fn new(search: SearchId) -> SearchStatus {
        SearchStatus {
            search,
            tasks: 0,
            committed: 0,
    ...
    pub fn apply(&mut self, record: &Record) {
        match &record.event {
            ...
            Event::SearchFinalized { .. } => {
                self.state = SearchState::Finalized;
                self.occupancy.clear();
            }
            ...
            Event::SearchInterrupted { .. } => {
                self.state = SearchState::Interrupted;
                self.occupancy.clear();
            }
            Event::WorkerBound {
                worker,
            ...
            Event::Checkpointed { .. } => {}
            // The phases of placing a search on a machine: where the search is in
...
pub fn status(config: &LoadedConfig) -> Result<SearchStatus> {
    Ok(status_records(
        config.search.id(),
...
/// The status of the search a loaded config describes, or the zeroed status when
/// there is no such search yet: no store at that root, or a search never driven in
/// it.
///
/// What a display seeded from prior progress wants: absence is the ordinary
/// case on a first search and reads as zeroes, while a corrupt journal or an I/O
...
pub fn seeded_status(config: &LoadedConfig) -> Result<SearchStatus> {
    let search = config.search.id();
    Ok(match journal::journaled(config)? {
        Some(records) => status_records(search, &records),
        None => SearchStatus::new(search),
    })
}

/// Merges `records` — a search's lifecycle events in append order — into the
/// status of `search`. The merge half of [`status`], over records from any
/// source: a journal read locally, or a stream from the host that drives the
/// search. It renders nothing through a domain, so it needs no format and cannot
/// fail.
pub fn status_records(search: SearchId, records: &[Record]) -> SearchStatus {
...
mod tests {
    ...
    fn search_id() -> SearchId {
        SearchId::from_hash(hash_bytes(b"status test search"))
    }
    ...
    /// The status a fresh search reaches by applying `records` in order.
    fn folded(records: Vec<Record>) -> SearchStatus {
        let mut status = SearchStatus::new(search_id());
        for record in &records {
            status.apply(record);
        }
        status
    ...
    fn a_store_that_does_not_exist_seeds_a_zeroed_status() -> Result<()> {
        // The ordinary first search: nothing has been driven, so a display seeded
        ...
        assert_eq!(seeded.search, config.search.id());
    ...
    fn a_corrupt_journal_surfaces_rather_than_seeding_zeroes() -> Result<()> {
        ...
        let mut writer = store.journal_writer(&config.search.id())?;
    ...
    fn checkpoint_degradation_counts_and_keeps_the_lease() {
        ...
        assert_eq!(status.occupancy.get(&0), Some(&occupancy("aa", 0)));
        assert!(matches!(status.state, SearchState::InProgress));
    }
    ...
    fn a_resume_segment_restates_tasks_and_resets_occupancy() {
        ...
        assert_eq!(status.state, SearchState::InProgress);
    ...
    fn run_level_events_set_the_terminal_state() {
        ...
        assert_eq!(finalized.state, SearchState::Finalized);

        let mut failed = SearchStatus::new(search_id());
        failed.apply(&rec(Event::SearchFailed {
            search: "00".repeat(32),
            task: "aa".to_string(),
            reason: "rejected".to_string(),
        }));
        ...
        let mut interrupted = SearchStatus::new(search_id());
        interrupted.apply(&rec(Event::SearchInterrupted {
            search: "00".repeat(32),
        }));
        assert_eq!(interrupted.state, SearchState::Interrupted);
    }

    #[test]
    fn the_journal_read_equals_a_replay_through_apply() -> Result<()> {
        let search = stub_config()?.id();
        ...
        let mut replay = SearchStatus::new(search);
        for record in &records {
            replay.apply(record);
        }
        assert_eq!(
        ...
        assert_eq!(replay.state, SearchState::Finalized);
    ...
    fn the_record_fold_equals_the_status_read_from_the_journal() -> Result<()> {
        ...
        let (_dir, config) = journal_with(&records)?;
        assert_eq!(
            status_records(config.search.id(), &records),
            status(&config)?,
