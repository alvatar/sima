...
/// What an invocation does when the program serving the run's format is a
/// different build from the one that drove the run before.
///
/// A run whose results and checkpoints came from another build is a run whose
...
pub enum BinaryChange {
    /// A changed program stops the run.
    Refuse,
    /// A changed program drives the run, and becomes what the next session
    /// compares against.
    Accept,
}

/// Compares the program now serving `run`'s format against the build the run
...
pub(crate) fn bind(
        ...
        && accept == BinaryChange::Refuse
    {
        return Err(Error::Validation(format!(
            "the program serving format {:?} changed since this run last ran: {} is now \
             {digest}, the run was driven by {previous}; stored results and checkpoints were \
    ...
    writer.append(
        &Record::stamped(Event::ProgramBound {
            format: run.format.as_str().to_string(),
...
/// The digest the run's last recorded program binding carries, or `None` for a
/// run that recorded none — a fresh run, or one whose format no config routed
/// when it ran.
///
...
mod tests {
    ...
    fn a_first_binding_records_the_build_and_compares_against_nothing() -> Result<()> {
        ...
        assert_eq!(bound_digests(&store, &run.id())?, [digest.to_string()]);
        Ok(())
    }
    ...
    fn an_unchanged_build_binds_again_under_a_refusal() -> Result<()> {
        ...
        // Every session records its binding, so the journal holds the run's
        // whole build history rather than only its first.
        assert_eq!(
            bound_digests(&store, &run.id())?,
    ...
    fn a_changed_build_under_a_refusal_names_both_digests_and_the_flag() -> Result<()> {
        ...
        // The refused session recorded nothing, so the run still names the
        // build that drove it.
        assert_eq!(bound_digests(&store, &run.id())?, [first.to_string()]);
        Ok(())
    }
    ...
    fn an_accepted_change_binds_the_new_build_for_the_next_session() -> Result<()> {
        ...
        bind(&store, &run, &routed(binary, &first), BinaryChange::Refuse)?;
        bind(&store, &run, &routed(binary, &second), BinaryChange::Accept)?;
        // The accepted build is what the next session compares against, so
        // running it again needs no flag.
        bind(&store, &run, &routed(binary, &second), BinaryChange::Refuse)?;
        assert_eq!(
            bound_digests(&store, &run.id())?,
    ...
    fn a_torn_line_before_the_binding_leaves_the_comparison_intact() -> Result<()> {
        ...
        store
            .journal_writer(&run.id())?
            .append("{\"ts_ms\":1,\"event\":\"no_such_event\"}")?;
        assert_eq!(last_digest(&store, &run.id())?, Some(digest.to_string()));
        Ok(())
    }
}
