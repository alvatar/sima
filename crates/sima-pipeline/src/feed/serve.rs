...
/// Serves the search `config` describes as a [`FollowFrame`] stream on `out`.
///
/// The config is loaded on this host, so the search id, the store path, and the
/// lock are all this host's. The stream opens with a `Hello` carrying the search
...
/// search never started, a corrupt journal — is written as a single `Fault`
...
fn serve(config: &Path, once: bool, out: &mut impl Write) -> Result<()> {
    ...
    // and a search never started there are errors, not empty streams.
    ...
    // The first `Records` frame carries the search's history and is always sent,
...
mod tests {
    ...
    use crate::fixtures::{served_config, served_search};
    ...
    fn a_snapshot_serves_hello_then_the_journal_then_complete() -> Result<()> {
        ...
        let (config, loaded) = served_search(dir.path(), &records)?;
        let lines = records
            .iter()
    ...
    fn the_opening_frame_carries_the_search_lock_holder() -> Result<()> {
        let dir = tempfile::tempdir().expect("temp dir");
        let (config, loaded) = served_search(dir.path(), &[committed("aa")])?;
        let store = Store::open(&loaded.store)?;
        let lock = store.acquire_search_lock(&loaded.search.id())?;
        let served = snapshot(&config)?;
        assert!(
            matches!(
                &served[0],
        ...
        drop(lock);
        // With the lock released the same snapshot reports a free search.
        assert!(matches!(
            &snapshot(&config)?[0],
