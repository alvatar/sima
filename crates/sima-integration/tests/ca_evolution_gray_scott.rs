...
/// 32x32 grid, `steps` per segment. `segments` renders the optional `[search]` key.
...
fn the_shipped_search_config_loads() -> Result<()> {
    ...
    // edit that breaks it fails here rather than only at search time.
    let loaded = load_example_variant(EXAMPLE, &shipped_example(EXAMPLE))?;
    assert_eq!(loaded.search.format.as_str(), "ca_evolution.gray_scott.v1");
...
mod on_device {
    ...
    fn a_segment_boundary_leaves_the_trajectory_byte_identical() -> Result<()> {
        ...
        // segmented search's store, because the second segment committed the same
        // bytes.
